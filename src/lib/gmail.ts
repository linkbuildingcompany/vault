...
/** Extract the HTML alternative from a Gmail payload, including nested MIME parts. */
export function extractHtmlBody(payload: {
  ...
  if (payload.parts && Array.isArray(payload.parts)) {
    type Part = { mimeType?: string | null; body?: { data?: string | null } | null; parts?: unknown[] | null };
    for (const part of payload.parts as Part[]) {
      const html = extractHtmlBody(part as Parameters<typeof extractHtmlBody>[0]);
      if (html) return html;
    }
  }
