...
export default function IntroductionsPage() {
  ...
  async function fetchData() {
    setFetching(true);
    try {
      const promises: Promise<Response>[] = [fetch("/api/vault/introductions")];
      if (isAdmin) promises.push(fetch("/api/vault/settings"));
      const [itemsRes, settingsRes] = await Promise.all(promises);
